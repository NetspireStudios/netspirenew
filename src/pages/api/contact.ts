...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // Get environment variables (using both process.env and import.meta.env for compatibility)
    const SMTP_HOST = process.env.SMTP_HOST || import.meta.env.SMTP_HOST || 'smtp.gmail.com';
    const SMTP_PORT = process.env.SMTP_PORT || import.meta.env.SMTP_PORT || '587';
    const SMTP_USER = process.env.SMTP_USER || import.meta.env.SMTP_USER || 'netspirestudios@gmail.com';
    const SMTP_PASS = process.env.SMTP_PASS || import.meta.env.SMTP_PASS || 'tjix fsoz iqgp xbsv';
    const CONTACT_EMAIL = process.env.CONTACT_EMAIL || import.meta.env.CONTACT_EMAIL || SMTP_USER;
    ...
    if (SMTP_HOST && SMTP_USER && SMTP_PASS) {
      try {
        ...
        await transporter.sendMail({
          from: `"NETSPIRE Contact Form" <${SMTP_USER}>`,
          to: CONTACT_EMAIL,
